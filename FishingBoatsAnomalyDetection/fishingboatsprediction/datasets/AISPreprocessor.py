...
class AISPreprocessor:
    def __init__(
        ...
        self.n_synthetic: float = self.synthetic_ratio / (
            1 - self.synthetic_ratio
        )  # per one trajectory, can be rational => probability is used in that case
        ...
        self.max_trajectory_sequence_len_to_predict = (
            max_trajectory_sequence_len_to_predict
        )
        self.min_trajectory_sequence_len_to_predict = (
            min_trajectory_sequence_len_to_predict
        )

        self.validation_size_ratio = validation_size_ratio
        self.test_size_ratio = test_size_ratio

        self.min_lat, self.max_lat, self.min_lon, self.max_lon, self.max_pseudo_lon = (
            None,
            None,
            None,
            None,
        ...
        self.local_lat_scale, self.local_lon_scale, self.local_pseudo_lon_scale = (
            None,
            None,
            None,
        )
        ...
        self.feature_cols = [
            self.cols.n_Latitude,
            self.cols.n_Longitude,
            self.cols.n_SOG,
            self.cols.n_COG,
        ]
    ...
    def run(self):
        all_boats_trajectories: Dict[str, pd.DataFrame] = (
            self.read_all_boats_trajectories()
        )  # Dict {mmsi: pd.DataFrame with trajectory}
        good_trajectories: List[pd.DataFrame] = (
            self.get_preprocessed_all_boats_trajectories(
                all_boats_trajectories.values()
            )
        )
        ...
        for i, t in enumerate(good_trajectories):
            self.display_trajectory(
                t, i, base_path=f"{self.output_dir}/sample_ship_trajectory_"
            )
            break

        if self.synthesize_flag:
            synthesized_trajectories: List[pd.DataFrame] = (
                self.get_synthesized_trajectories(good_trajectories)
            )

        good_trajectories.extend(synthesized_trajectories)

        self.get_latlon_box(good_trajectories)
        normalized_trajectories: List[pd.DataFrame] = self.get_normalized_trajectories(
            good_trajectories
        )

        normalized_trajectories: List[pd.DataFrame] = self.create_prediction_masks(
            normalized_trajectories
        )

        normalized_trajectories: List[pd.DataFrame] = self.filter_long_trajectories(
            normalized_trajectories
        )
...
"""
    ...
    def validate_latlon_scale(self):
        if self.max_pseudo_lon is None:
            raise Exception(
                "self.max_pseudo_lon is None. Should be calculated for validate_latlon_scale"
            )
        calculated_latlon_scale = max(
            self.max_lat - self.min_lat, self.max_pseudo_lon - self.min_lon
        )
        if self.latlon_scale is None:
            self.latlon_scale = calculated_latlon_scale
        if self.latlon_scale < calculated_latlon_scale:
            raise Exception(
                f"Calculated latlon_scale {calculated_latlon_scale} is bigger than given latlon_scale {self.latlon_scale}!!! That's not possible"
            )
...
"""
    ...
    def get_preprocessed_all_boats_trajectories(self, trajectories: List[pd.DataFrame]):
        ...
        for trajectory in trajectories:
            ...
            current_good_trajectories = [
                seq
                for seq in trajectory_sequences
                if len(seq) >= self.trajectory_sequence_len
            ]
        ...
        for t in pre_good_trajectories:
            ...
            current_good_trajectories = [
                t[
                    i
                    * self.trajectory_sequence_len : (i + 1)
                    * self.trajectory_sequence_len
                ]
    ...
    def normalize_trajectory(self, trajectory: pd.DataFrame):
        ...
        trajectory[self.cols.n_Latitude] = (
            trajectory[self.cols.Latitude] - cur_lat_min
        ) / self.local_lat_scale
        trajectory[self.cols.n_Longitude] = (
            trajectory[self.cols.Pseudo_Longitude] - cur_pseudo_lon_min
        ) / self.local_pseudo_lon_scale
        trajectory[self.cols.n_SOG] = (
            (trajectory[self.cols.SOG] / SPEED_MAX < 1)
            * trajectory[self.cols.SOG]
            / SPEED_MAX
        ) + (trajectory[self.cols.SOG] / SPEED_MAX > 1)
        trajectory[self.cols.n_COG] = trajectory[self.cols.COG] / MAX_DEGREES
        trajectory[self.cols.n_Heading] = (
            (trajectory[self.cols.Heading] / MAX_DEGREES < 1)
            * trajectory[self.cols.Heading]
            / MAX_DEGREES
        ) + 511 * (trajectory[self.cols.Heading] / MAX_DEGREES > 1)

        # center self.cols.n_Latitude and self.cols.n_Longitude around 0.5
        trajectory[self.cols.n_Latitude] = trajectory[self.cols.n_Latitude] + (
            0.5 - trajectory[self.cols.n_Latitude].max() / 2
        )
        trajectory[self.cols.n_Longitude] = trajectory[self.cols.n_Longitude] + (
            0.5 - trajectory[self.cols.n_Longitude].max() / 2
        )
        return trajectory
    ...
    def get_normalized_trajectories(
        ...
        for trajectory in trajectories:
            ...
            trajectory[self.cols.Pseudo_Longitude] = self.min_lon + (
                trajectory[self.cols.Longitude] - self.min_lon
            ) * np.cos(np.radians(trajectory[self.cols.Latitude]))
        ...
        normalized_trajectories = [
            self.normalize_trajectory(trajectory) for trajectory in trajectories
        ]
        return normalized_trajectories

    def create_prediction_masks(
        self, trajectories: List[pd.DataFrame]
    ) -> List[pd.DataFrame]:

        for trajectory in trajectories:
            n_predict = np.random.randint(
                self.min_trajectory_sequence_len_to_predict,
                self.max_trajectory_sequence_len_to_predict + 1,
            )
            start_range = range(
                self.prediction_buffer,
                self.trajectory_sequence_len - n_predict - self.prediction_buffer + 1,
            )
            start_idx = np.random.choice(start_range)

            mask = np.zeros(
                self.trajectory_sequence_len, dtype=bool
    ...
    def filter_long_trajectories(
        self, trajectories: List[pd.DataFrame]
    ) -> List[pd.DataFrame]:
        distances_to_bounding_box = (
            [t[self.cols.n_Latitude].min() for t in trajectories]
            + [t[self.cols.n_Longitude].min() for t in trajectories]
            + [1 - t[self.cols.n_Latitude].max() for t in trajectories]
            + [1 - t[self.cols.n_Longitude].max() for t in trajectories]
        )
        ...
        filtered_and_scaled = []
        for t in filtered:
            t[self.cols.n_Latitude] = (t[self.cols.n_Latitude] - 0.5) / (
                upper_bound - lower_bound
            ) + 0.5
            t[self.cols.n_Longitude] = (t[self.cols.n_Longitude] - 0.5) / (
                upper_bound - lower_bound
            ) + 0.5
    ...
    def synthesize_trajectory(self, trajectory: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.rotate_trajetories:
            ...
            synthetic_trajectory = self.rotate_trajectory(
                synthetic_trajectory, angle_deg
            )

        if self.shitf_trajectories:
            lat_shift, lon_shift = self.get_random_bounded_latlon_shifts(
                synthetic_trajectory
            )
            synthetic_trajectory = self.shift_trajectory(
                synthetic_trajectory, lat_shift, lon_shift
            )
    ...
    def get_synthesized_trajectories(
        self, trajectories: List[pd.DataFrame]
    ) -> List[pd.DataFrame]:
    ...
    def get_sampled_trajectory(self, trajectory: pd.DataFrame) -> pd.DataFrame:
        ...
        valid_sampling_times = [
            ...
            if any(abs(trajectory.index - t) <= self.sample_T)
        ]
        ...
        trajectory_sampled.rename(
            columns={"index": self.cols.Sampled_Date}, inplace=True
        )
        return trajectory_sampled

    def get_trajectory_sequences(
        self, trajectory_sampled: pd.DataFrame, time_column_name=None
    ) -> List[pd.DataFrame]:
    ...
    def rotate_trajectory(
        ...
        pivot_lat = (
            trajectory[self.cols.Latitude].max() + trajectory[self.cols.Latitude].min()
        ) / 2
        pivot_lon = (
            trajectory[self.cols.Longitude].max()
            + trajectory[self.cols.Longitude].min()
        ) / 2
        ...
        enu_mat = AISPreprocessor.enu_matrix(
            pivot_lat, pivot_lon
        )  # Rows: [east, north, up]
        ...
        ecef_coords = np.array(
            [
                AISPreprocessor.geodetic_to_ecef(lat, lon)
                for lat, lon in zip(
                    trajectory[self.cols.Latitude], trajectory[self.cols.Longitude]
                )
            ]
        )
        ...
        new_lat_lon = np.array(
            [
                AISPreprocessor.ecef_to_geodetic(x, y, z)[:2]
                for x, y, z in new_ecef_coords
            ]
        )
        trajectory[[self.cols.Latitude, self.cols.Longitude]] = new_lat_lon
        trajectory[self.cols.COG] = (
            trajectory[self.cols.COG] + angle_deg
        ) % MAX_DEGREES
        # rotate Heading only if it is not equal to 511 (reference: https://api.vtexplorer.com/docs/response-ais.html)
        trajectory[self.cols.Heading] = (trajectory[self.cols.Heading] != 511) * (
            trajectory[self.cols.Heading] + angle_deg
        ) % MAX_DEGREES + (trajectory[self.cols.Heading] == 511) * 511

        return trajectory

    def get_random_bounded_latlon_shifts(self, trajectory):
        min_lat, max_lat = (
            trajectory[self.cols.Latitude].min(),
            trajectory[self.cols.Latitude].max(),
        )
        min_lon, max_lon = (
            trajectory[self.cols.Longitude].min(),
            trajectory[self.cols.Longitude].max(),
    ...
    def display_trajectory(
        ...
        trajectory: pd.DataFrame,
        i=0,
        base_path="data/good_trajectories/ship_trajectory_",
    ):
        # is_synthetic = False
        ...
        zoom_level = max(
            1, min(18, zoom_level)
        )  # Adjust 1 and 18 as needed for your data.
        ...
        if self.cols.n_Latitude in trajectory.columns:
            fig = px.line(
                trajectory,
                x=self.cols.n_Longitude,
                y=self.cols.n_Latitude,
                title="Trajectory",
            )
    ...
    @staticmethod
    def plot_individual_trajectory_comparisons(
        original, reconstructed, output_dir, x_col=0, y_col=1, mask_col=4
    ):
        """
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        showlegend=(
                            True
                            if (
                                (mask[i] == 1 and i == np.where(mask == 1)[0][0])
                                or (mask[i] == 0 and i == np.where(mask == 0)[0][0])
                            )
                            else False
                        ),
    ...
    @staticmethod
    def plot_probabilistic_trajectory_comparisons(
        original,
        reconstructed_list,
        output_dir,
        x_col=0,
        ...
        for sample_idx in range(samples_num):
            ...
            try:
                ...
                print(
                    f"KDE calculation failed for sample {sample_idx} - possibly all points are identical. Error: {e}"
                )
                continue
...
if __name__ == "__main__":
    ...
    # aisp = AISPreprocessor(
    #     dataset_path="data/FishingKoreaAIS/Dynamic_*.csv",  # dataset_path = 'data/testAIS/Dynamic_*.csv',
